...
本工程解决**数据集自动标注**的问题：在无标注或粗标注的图片数据（如 AP-36k 动物姿态数据集）上，用两类强检测器联合生成高质量检测标签。

- **Co-DETR** 用作高精度 pseudo-GT 来源（Swin-L backbone，COCO AP ~66.0）
- **GroundingDINO** 用文本提示（如 `"animal."`）处理开放词汇 / 长尾类别
- 通过 Co-DETR ↔ YOLO 的 FP/FN 对比工作流，筛出 YOLO 的误检/漏检样本，为下游 finetune 提供有针对性的训练数据
